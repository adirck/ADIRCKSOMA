CREATE PROCEDURE [dbo].[LX_GERA_NFE_SEFAZ_4_00]  @NOTAFISCAL VARCHAR(15), @SERIENOTA VARCHAR(6), @FILIALNOTA VARCHAR(25), @RETORNA_XML BIT = 0, @APP_VERSAO VARCHAR(20) = NULL
	WITH ENCRYPTION  
AS
-- AZZ.01.01	- Eli Rodrigues - 21/05/2025 - Incluindo TIPO_INTEGRA = 2 para retornar dados do PATH = "card" (SomaBox)
----#GS:01.02# - RF-#GS:01.02# - Alterado view W_NOTAS_REFERENCIADAS_DEV para funÃ§Ã£o FN_NOTAS_REFERENCIADAS_DEV
-- PRODSHOP-6652 - EDER SILVA         - #87# - 22/06/2021 - Tratamento para calcular o vICMSUFDest em notas de troca 
-- POSSP-5683	 - Wesley Batista     - #86# - 11/06/2021 - Correção para não deuplicar o valor da tag pIPI
-- POSSP-5446	 - Gilvano Santos     - #85# - 19/05/2021 - Preenchimento das tags vIPI, pDevol e vIPIDevol para devolução via transferencia.
-- POSSP-5193	 - Gilvano Santos     - #84# - 31/03/2021 - Reshop - Venda de produto mais Vitrine com Digite Parcelas está gerando Tpag 05 e valor apenas do produto Vitrine (e rejeição NFe: ausencia de troco)
-- POSSP-5157	 - Gilvano Santos     - #83# - 31/03/2021 - Adequação a NT 2020.006 Intermediador da Operação – Marketplace e outros
/* SUSTSP-2124   - Wendel Crespigio   - #82# - 18/01/2021 - Correção para quando cliente usa FCP e o valor do imposto e inferior a 0.00 centavos calculado como 0,0001
-- SUSTSP-1013   - Fillipi Ramos      - #81# - 18/08/2020 - Correção para não deixar de gerar a DetPag quando for uma nf-e de devolução de venda.
-- MODASP-10432  - Juliana Nascimento - #80# - 27/01/2020 - Implementar versão do Service Pack/Hotfix na tag <verProc>
...
-- MODASP-6411   - Wendel Crespigio   - #77# - 23/09/2019 - Ajuste na seleção da tag vICMS para "51". Valor ICMS_BST será calculado pelo sistema e não mais pela procedure.
-- MODASP-6320   - Wendel Crespigio   - #76# - 19/09/2019 - Ajuste na seleção do IMCSST90. Bloco de seleção do imposto desonerado calculando apenas quando ocorrer ST. Ajuste para ser calculado em TODAS as situações.
...
DECLARE --@NOTAFISCAL VARCHAR(7), @SERIENOTA VARCHAR(6), @FILIALNOTA VARCHAR(25),  
		...
		@LINX_VERSAO AS VARCHAR(50), @NT_2020_006 AS BIT -- #80# #83#

		SELECT @CNF_INVALIDO = 0
		SELECT @CODIGO_FILIAL = CODIGO_FILIAL FROM LOJAS_VAREJO WHERE FILIAL = @FILIALNOTA -- #53#

SELECT @LINX_VERSAO = VALOR_ATUAL FROM PARAMETROS WHERE PARAMETRO = 'VERSAO_SERVICE_PACK_LOJA' -- #80#
...
--Inicio #83#
select @NT_2020_006 = case when (GETDATE() > CONVERT(DATETIME, VALOR_ATUAL,103)) or
			(getdate() > cast('20210831' as datetime)) then 1 else 0 end
from PARAMETROS 
where PARAMETRO = 'DATA_INI_NT2020_006' 
--Fim #83#
...
------- IDE 	
	...
	-- Edson -  removido	Bloco NFREF		
			...
			CASE WHEN ( @NT_2020_006 = 0 or RTRIM(NFE.INDICA_PRESENCA_COMPRADOR) not IN (1,2,3,4,9)) THEN NULL ELSE
					( CASE WHEN RTRIM(NFE.INDICA_PRESENCA_COMPRADOR) IN (1,2,3,4,9) 
					AND ISNULL(NFE.INTERMERDIADOR,0) > 0 THEN '1' ELSE '0' END ) END										"indIntermed",  -- #83#
			...
			@LINX_VERSAO "verProc", -- VERSAO DO LINX / RETAGUARDA OU LOJA*/
			/* #80# - Fim */
			...
			-- EDSON -- NF ORIGEM VINCULADA A NF TROCA e CANCELAMENTO	#16#
				( 
					  SELECT 
						...
						FROM [dbo].[FN_NOTAS_REFERENCIADAS_DEV] (@NOTAFISCAL, @SERIENOTA, NULL) AS W_NOTAS_REFERENCIADAS_DEV --#GS:01.02#
				...
				( SELECT  	
					...
					( 
						SELECT 	RTRIM(NFE_ITENS.TRIBUT_ORIGEM)					"orig",
								...
								CASE WHEN NFE_ITENS.FECP_VALOR    > 0 THEN CONVERT(NUMERIC(15,2),NFE_ITENS.FECP_VALOR)   ELSE CASE WHEN NFE_ITENS.FECP_ALIQUOTA > 0 THEN 0 ELSE NULL END END	"vFCP" --#82#
								--#45#
				...
				( SELECT  	
					...
					( 
						SELECT 	RTRIM(NFE_ITENS.CST_IPI)						"CST",
								CASE WHEN @TIPO_OPERACAO in ('T') AND @FIN_EMISSAO_NFE = 4 
									then CONVERT(NUMERIC(15,2), 0.00)
								else CONVERT(NUMERIC(15,2),NFE_ITENS.IPI_BASE) 
								end as											"vBC",  --#85#
								CASE WHEN @TIPO_OPERACAO in('T') AND @FIN_EMISSAO_NFE = 4 
									then CONVERT(NUMERIC(7,4), 0.00) 
								else CONVERT(NUMERIC(7,4),NFE_ITENS.IPI_ALIQUOTA) 
								end as											"pIPI", --#85#
								--CONVERT(NUMERIC(7,4),NFE_ITENS.IPI_ALIQUOTA)	"pIPI", --#13# -- #86#
								NULL											"qUnid",				 
								NULL											"vUnid",				 
								CASE WHEN @TIPO_OPERACAO in ('D','T') AND @FIN_EMISSAO_NFE = 4 --#85# 
...
-------------------------------- COFINS ------------------------------------------
				...
				( 
						...
						AND SUBSTRING(NFE_ITENS.CODIGO_FISCAL_OPERACAO,1,1) in ('6','2')  --#87#
...
------------ impostoDevol 
					(SELECT 
							(   SELECT   pDevol AS "pDevol" FROM 
								...
								AND	NFE.TIPO_OPERACAO in ('D','T')   --#65# Filtro para operação de Devolução --#85#
...
--- PAGAMENTO #45#  #46#
			...
			SELECT 
				(
					SELECT 	  /*NFE.TIPO_PGTO_NFE																"indPag",*/ -- 0 - PAGTO A VISTA, 1 - PAGTO A PRAZO E 2 - OUTROS. --#69#
							  ...
							  --CONVERT(NUMERIC(15,2),RTRIM(W_INFORMACAO_PAGAMENTO.TROCO))  + --#48##54# #84#	
		...
		/*AZZ.01.01*/ 				CASE WHEN W_INFORMACAO_PAGAMENTO.TIPO_INTEGRA IS NOT NULL AND W_INFORMACAO_PAGAMENTO.TIPO_INTEGRA In (1,2) THEN CONVERT(varchar(14),RTRIM(W_INFORMACAO_PAGAMENTO.CNPJ_CREDENCIADORA)) END	"CNPJ",  --#51#
		/*AZZ.01.01*/				CASE WHEN W_INFORMACAO_PAGAMENTO.TIPO_INTEGRA IS NOT NULL AND W_INFORMACAO_PAGAMENTO.TIPO_INTEGRA In (1,2) THEN CONVERT(varchar(2),RTRIM(W_INFORMACAO_PAGAMENTO.BANDEIRA)) END				"tBand", --#51#
		/*AZZ.01.01*/				CASE WHEN W_INFORMACAO_PAGAMENTO.TIPO_INTEGRA IS NOT NULL AND W_INFORMACAO_PAGAMENTO.TIPO_INTEGRA In (1,2) THEN RTRIM(W_INFORMACAO_PAGAMENTO.AUTORIZACAO) END								"cAut" 
								WHERE W_INFORMACAO_PAGAMENTO.NF = @NOTAFISCAL AND 
									  W_INFORMACAO_PAGAMENTO.SERIE_NF = @SERIENOTA AND
				...
				LEFT join [DBO].[FX_CALCULA_RATEIO_PARCELAS](@qtde_parcela,
					case 
						...
						then CONVERT(NUMERIC(15,2),NFE.ICMS_ST + NFE.ICMS_STR) + CONVERT(NUMERIC(15,2),NFE.FECP_ST) else 0 end) b -- #81# alterado de INNER para LEFT na [DBO].[FX_CALCULA_RATEIO_PARCELAS]
				...
				(
					SELECT	case when (sum((case when W_INFORMACAO_PAGAMENTO.VALOR > 0.00 then W_INFORMACAO_PAGAMENTO.VALOR else 0 end )) - NFE.VALOR_TOTAL) <= 0.00 then null
							else CONVERT(NUMERIC(15,2), sum((case when W_INFORMACAO_PAGAMENTO.VALOR > 0.00 then W_INFORMACAO_PAGAMENTO.VALOR else 0.00 end )) - NFE.VALOR_TOTAL) end "vTroco" --#84#
					--case when CONVERT(NUMERIC(15,2),RTRIM(SUM(W_INFORMACAO_PAGAMENTO.TROCO))) >0 then  CONVERT(NUMERIC(15,2),RTRIM(SUM(W_INFORMACAO_PAGAMENTO.TROCO)))--#55# --#84#
					...
					--else  CONVERT(NUMERIC(15,2),RTRIM(SUM(W_INFORMACAO_PAGAMENTO.TROCO)))	*-1 end		"vTroco" --#48# --#84#
...
--#45# -- #46#
----#83# -- Informações do Intermediador da Transação
			(
				SELECT	case WHEN @NT_2020_006 = 0 THEN NULL ELSE rtrim(ltrim(NFE.INTERMERDIADOR_CNPJ)) end AS "CNPJ",
						case WHEN @NT_2020_006 = 0 THEN NULL ELSE rtrim(ltrim(NFE.INTERMERDIADOR_IDCADINTTRAN)) end AS "idCadIntTran"
				WHERE RTRIM(NFE.INDICA_PRESENCA_COMPRADOR) IN (1,2,3,4,9) 
					AND ISNULL(NFE.INTERMERDIADOR,0) > 0
				FOR XML PATH(''), TYPE
			) as infIntermed,		
----#83# -- Informações do Intermediador da Transação		
