ALTER TRIGGER [dbo].[LXU_LOJA_SAIDAS_PRODUTO] ON [dbo].[LOJA_SAIDAS_PRODUTO] FOR UPDATE NOT FOR REPLICATION AS
-- UPDATE trigger on LOJA_SAIDAS_PRODUTO
--05/06/2025 - Carlos G. Gonçalves (Megasult) - Removido a seguinte customização anterior SS#1: ##31/05/2016 - Felipe Silva SS#1 - Retirado select que verificava quantidades do deleted e subtraia do inserted, pois quando salvava uma saida sem encerrar e finalizava depois a trigger não alterava o estoque.## Pois estava causando problemas de baixar 2 vezes o estoque.
...
/* PRODUTO_CORES R/1191 LOJA_SAIDAS_PRODUTO ON CHILD UPDATE RESTRICT */
  if
    update(PRODUTO) or
    update(COR_PRODUTO)
  begin
    ...
    begin
      select @errno  = 30007,
             @errmsg = 'Impossível atualizar  "LOJA_SAIDAS_PRODUTO" porque "PRODUTO_CORES" não existe.'
      goto error
    end
  end

/* LOJA_SAIDAS R/1190 LOJA_SAIDAS_PRODUTO ON CHILD UPDATE RESTRICT */
  if
  ...
  begin
    ...
    begin
      select @errno  = 30007,
             @errmsg = 'Impossível atualizar  "LOJA_SAIDAS_PRODUTO" porque "LOJA_SAIDAS" não existe.'
      goto error
    end
  end
...
--- Verifica bloqueio por contagem ------------------------------------------------------------------------------
	...
	BEGIN
		IF EXISTS (SELECT 
			...
			FROM 
				Deleted 
				INNER JOIN LOJA_SAIDAS ON Deleted.FILIAL = LOJA_SAIDAS.FILIAL AND Deleted.ROMANEIO_PRODUTO = LOJA_SAIDAS.ROMANEIO_PRODUTO
				INNER JOIN ESTOQUE_PRODUTOS ON ESTOQUE_PRODUTOS.FILIAL = Deleted.FILIAL AND ESTOQUE_PRODUTOS.PRODUTO = Deleted.PRODUTO AND ESTOQUE_PRODUTOS.COR_PRODUTO = Deleted.COR_PRODUTO 
			WHERE 
				LOJA_SAIDAS.EMISSAO < ESTOQUE_PRODUTOS.DATA_AJUSTE)
		BEGIN
...
/*---LINX-ETL------------------------------------------------------------------------------------------*/
	IF EXISTS (SELECT 1 FROM LOJA_SAIDAS where TIPO_ENTRADA_SAIDA <> '06')	
		UPDATE LOJA_SAIDAS  
	    ...
	    WHERE	INSERTED.ROMANEIO_PRODUTO = LOJA_SAIDAS.ROMANEIO_PRODUTO AND
				...
				ISNULL(LOJA_SAIDAS.LX_STATUS_SAIDA,0) = 3
...
--- DATA PARA TRANSFERENCIA -------------------------------------------------------------------------------------
	IF EXISTS (SELECT 1 FROM LOJA_SAIDAS where TIPO_ENTRADA_SAIDA <> '06')	
		...
		BEGIN
			...
			WHERE 	LOJA_SAIDAS_PRODUTO.ROMANEIO_PRODUTO = INSERTED.ROMANEIO_PRODUTO and
				LOJA_SAIDAS_PRODUTO.FILIAL = INSERTED.FILIAL and
				LOJA_SAIDAS_PRODUTO.PRODUTO = INSERTED.PRODUTO and
				LOJA_SAIDAS_PRODUTO.COR_PRODUTO = INSERTED.COR_PRODUTO 
				AND (INSERTED.DATA_PARA_TRANSFERENCIA IS NULL 
				OR LOJA_SAIDAS_PRODUTO.DATA_PARA_TRANSFERENCIA = INSERTED.DATA_PARA_TRANSFERENCIA)
		END
-----------------------------------------------------------------------------------------------------------------
