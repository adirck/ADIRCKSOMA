...
ALTER PROCEDURE [dbo].[LX_GERA_NFE_SEFAZ_3_10]  @NOTAFISCAL VARCHAR(15), @SERIENOTA VARCHAR(6), @FILIALNOTA VARCHAR(25), @RETORNA_XML BIT = 0, @APP_VERSAO VARCHAR(20) = NULL
	  
AS										
/*
-- Bruno Silva (SS) - (23/03/2017) - Replicação para SP10 - 17/02/2015 - Leandro Rocha (SS) - #SS02# - Efetuado convert no campo da serie das views de referenciamento.
-- Bruno Silva (SS) - (23/03/2017) - Replicação para SP10 - 01/10/2015 - Felipe Silva - #SS01# - Retirada clausula filial = nome_clifor, para conseguir referenciar notas de outras filiais e de outros clientes e retirada criptografia da procedure.
...
-- DM 2454 - Giedson Silva #40# - (20/04/2016) - - Retirada da soma do DICMS_DEST_VALOR + FECP_DEST_VALOR
...
-- DM19665 ou 129 - Giedson Silva  - #37# - (03/03/2016) - Tratamento para gerar as informações do produto NVE.
...
-- TP10769689 - Diego.Moreno     - #34# - (02/12/2015) - Deve gerar as tags mesmo zerado conforme NT 2015.003 1.40.
-- TP10769689 - Diego.Moreno     - #33# - (02/12/2015) - Austes referente ao documento Nota Técnica 2015.003 - v 1.40.
-- TP10769689 - Diego.Moreno     - #32# - (26/11/2015) - Deve ser gerado um grupo a parte do ICMS para a partilha e ID_DESTINO_OPERACAO deve ser 2.
-- TP10769689 - Gilvano.Santos   - #31# - (18/11/2015) - Adequação NT 2015.003 - Partilha do ICMS
...
-- TP8304984 - JORGE.DAMASCO - #20# - (14/04/2015) - IMPLEMENTAÇÃO PARA QUANDO SE TRATAR DE UMA NOTA DE DEVOLUÇÃO P/ RESERVA/CONSIGNAÇÃO.
...
DECLARE --@NOTAFISCAL VARCHAR(7), @SERIENOTA VARCHAR(6), @FILIALNOTA VARCHAR(25),  
		...
		@OBS_INTERESSE_FISCO VARCHAR(2000), @COD_MUNICIPIO_IBGE CHAR(7), @TIPO_DOC_CHAVE CHAR(2), @CLIFOR_FILIALNOTA CHAR(6),@NF_NUMERO_REFERENCIADA char(1) --#35#
...
IF @CHAVEACESSO IS NULL
	BEGIN
		...
		IF RTRIM(@VERSAO_CHAVE_NFE) = '2.00' OR RTRIM(@VERSAO_CHAVE_NFE) = '3.10'  -- VERSAO 2.00
			BEGIN
					  ...
					  SELECT 
						...
						WHERE --W_NOTAS_REFERENCIADAS.CHAVE_NFE IS NOT NULL AND
							W_NOTAS_REFERENCIADAS.NF = @NOTAFISCAL
								AND convert(char(6), W_NOTAS_REFERENCIADAS.SERIE) = @SERIENOTA --#SS02#
								--AND W_NOTAS_REFERENCIADAS.FILIAL = @FILIALNOTA
								AND W_NOTAS_REFERENCIADAS.FILIAL = CASE WHEN @TIPO_DOC = 'E' 
								and (@TIPO_DOC_CHAVE != 'L'/*#26#*/ Or @NF_NUMERO_REFERENCIADA = 1)
				...
				( 
					  SELECT 
						...
						WHERE W_NOTAS_REFERENCIADAS_DEV.NF = @NOTAFISCAL
								...
								/* AND W_NOTAS_REFERENCIADAS_DEV.FILIAL  = @NOME_CLIFOR --#18# #SS01# */  
...
------- PROD		
			(
				...
				SELECT DBO.FX_REPLACE_CARACTER_ESPECIAL_NFE(DEFAULT,NFE_ITENS.CODIGO_ITEM)											"cProd",
					NFE_ITENS.EAN																									"cEAN",	-- ???) CRIAR FUNCAO
		...
		( 
				...
				FOR XML PATH(''), TYPE),
					...
					NFE_ITENS.EAN																									"cEANTrib",	-- ???) CRIAR FUNCAO												
				...
				( SELECT  	
					...
					( 
						...
						(	SELECT 
								3													"modBC",
								CONVERT(NUMERIC(7,4),NFE_ITENS.PREDBC)				"pRedBC", -- PERCENTUAL DE REDUCAO DA MARGEM DE CALCULO (PORCENT_REDUCAO_DE_BASE)--#13#
								--CONVERT(NUMERIC(15,2),NFE_ITENS.ICMS_BST_BASE)	"vBC", #28#
								--CONVERT(NUMERIC(7,4),NFE_ITENS.ICMS_BST_ALIQUOTA)	"pICMS",--#13##28#
								CONVERT(NUMERIC(15,2),NFE_ITENS.ICMS_BASE)			"vBC",--#28#
								CONVERT(NUMERIC(7,4),NFE_ITENS.ICMS_ALIQUOTA)		"pICMS", --#28#
								--#3.1#
								--CONVERT(NUMERIC(15,2),NFE_ITENS.ICMS_BST)			"vICMSOp",
								--													"pDif",
								--													"vICMSDif", 
								--#3.1#
								
								--#27# -- Removendo comentário. Tags necessárias para ICMS diferido. (Tribut_ICMS 51)
				 ...
				 --( 
					...
					--	WHERE NFE.ID_DESTINO_OPERACAO = 1
				...
				(
				 ...
				 ( 
						SELECT 	CONVERT(NUMERIC(15,2),NFE_ITENS.DICMS_BASE)							"vBCUFDest",
								...
								CONVERT(NUMERIC(7,4),NFE_ITENS.PART_ICMS_DEST_ALIQUOTA)				"pICMSInterPart",
								CONVERT(NUMERIC(15,2),NFE_ITENS.FECP_DEST_VALOR) 					"vFCPUFDest",					
								CONVERT(NUMERIC(15,2),NFE_ITENS.DICMS_DEST_VALOR)					"vICMSUFDest", -- #33# + CONVERT(NUMERIC(15,2),NFE_ITENS.FECP_DEST_VALOR),#40#                      
								CONVERT(NUMERIC(15,2),NFE_ITENS.DICMS_ORIG_VALOR)					"vICMSUFRemet"
						WHERE NFE.ID_DESTINO_OPERACAO = 2
						AND NFE.UF_EMITENTE <> NFE.UF
						AND SUBSTRING(NFE_ITENS.CODIGO_FISCAL_OPERACAO,1,1) = '6'  
						AND NFE.INDICADOR_IE_DESTINARARIO = 9 --#34#
						AND NFE_ITENS.DICMS_BASE > 0 -- #42#
						--AND NFE_ITENS.ICMS_DEST_ALIQUOTA > NFE_ITENS.ICMS_ALIQUOTA --#34#
						FOR XML PATH(''), TYPE
				  ) AS ICMSUFDest
				   --#32# Fim 				  
...
-------------ICMSTOT		
			(
				SELECT CONVERT(NUMERIC(15,2),NFE.ICMS_BASE)																		"vBC",
					...
					CONVERT(NUMERIC(15,2),NFE.DICMS_DEST)																		"vICMSUFDest", --#31# #33# + CONVERT(NUMERIC(15,2),NFE.FECP_DEST)#40#
