...
-- Felipe Carvalho - (SS02) - 09/08/2018 - CRADO NOVO SELECT PARA RETORNAR SE A CHAVE REFENCIADA ESTIVER NO CAMPO EXCLUSIVO ANILAME DA TABELA LOJA_VENDA_TROCA_ORIGEM 
-- Felipe Carvalho - (SS01) - 09/08/2018 - CORREÇÃO PARA O SELECT ORGINAL NÃO RETORNAR SE A CHAVE REFENCIADA ESTIVER NO CAMPO EXCLUSIVO ANILAME DA TABELA LOJA_VENDA_TROCA_ORIGEM 
(
...
--**********************************************************************************************************************************************************************
			...
			INNER JOIN LOJA_VENDA_TROCA_ORIGEM C   ON C.CODIGO_FILIAL = C.CODIGO_FILIAL 
												  AND C.DATA_VENDA = C.DATA_VENDA 
												  AND C.TICKET = C.TICKET /*SS01*/
			
			where	A.NUMERO_FISCAL_TROCA = @NOTAFISCAL
			AND		A.SERIE_NF_ENTRADA =	@SERIENOTA
			--and A.VENDA_FINALIZADA = 1 --#2#
			AND     C.CHAVE_NFE_ORIGEM IS NULL /*SS01*/
...
UNION ALL
	...
	INNER JOIN (SELECT	C.NUMERO_NF_TRANSFERENCIA AS  NF_ORIGEM, 
						C.SERIE_NF_ENTRADA AS SERIE_NF_ORIGEM, 
						E.CODIGO_FILIAL AS CODIGO_FILIAL_ORIGEM, 
						C.CHAVE_NFE AS CHAVE_NFE_ORIGEM,
						A.NUMERO_NF_TRANSFERENCIA AS NF_SAIDA, 
						A.SERIE_NF AS SERIE_NF_SAIDA, 
						F.CODIGO_FILIAL AS CODIGO_FILIAL_SAIDA
				FROM LOJA_SAIDAS A
				...
				GROUP BY C.NUMERO_NF_TRANSFERENCIA, C.SERIE_NF_ENTRADA, E.CODIGO_FILIAL, C.CHAVE_NFE, A.NUMERO_NF_TRANSFERENCIA , A.SERIE_NF , F.CODIGO_FILIAL )  NF_ORIGEM
	...
	AND B.NF_NUMERO = @NOTAFISCAL
	AND B.SERIE_NF  = @SERIENOTA
