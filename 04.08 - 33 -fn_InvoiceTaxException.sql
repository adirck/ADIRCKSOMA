/****** Object:  UserDefinedFunction [dbo].[fn_InvoiceTaxException]    Script Date: 01/10/2024 16:43:07 ******/
SET ANSI_NULLS ON
GO
...
ALTER Function [dbo].[fn_InvoiceTaxException] (@OperationType int, @DestinationCode char(14), @IndicadorCFOP int, @TaxFlag char(1), @StoreName varchar(25), @DocumentDate datetime, @TributOrigem char(3), @CFOP char(4), @ItemCode varchar(40), @DestinationName varchar(25), @ItemType char(1), @FiscalOperation varchar(15), @StoreCode char(6), @sat bit = 0, @DestinationUF varchar(2)) 
RETURNS INT
AS
-- 02/10/2024 - Jonathas Nogueira - #GS:01.01# - Ajuste do campo TRIBUT_ORIGEM de 1 para 15

-- 02/07/2024 - Leonardo Marques - POSSP-14528 - #05# - Linx POS - Atualizar a seleção de exceção de imposto em relação ao Linx ERP
--                               - O parâmetro @DestinationUF da fn_InvoiceTaxExceptionPOS foi adicionado neste fonte para ficar somente uma function.
-- 04/04/2024 - Fillipi Ramos    - POSSP-14010- #04# - Criação da funcion fn_InvoiceTaxExceptionPOS para utilização para pegar exeçôes de imposto a partir de uma UF passada por parâmetro (@DestinationUF)
-- 02/10/2023 - Fernando Marinho - PRODSHOP-21576 - #03# - Tratamento para filtro dos campos DT_INICIO_VIGENCIA e DT_FIM_VIGENCIA
...
BEGIN
	...
	FROM 
		(SELECT
			...
			CASE WHEN A.UF = ISNULL(@DestinationUF, D.UF) THEN 1 ELSE 0 END + -- #04# #05#
	    ...
	    CASE WHEN A.TRIBUT_ORIGEM = @TributOrigem THEN 15 ELSE 0 END +  /*#GS:01.01#*/  
			...
			CASE WHEN E.PRODUTO IS NOT NULL AND A.CODIGO_ITEM = E.PRODUTO THEN 15 ELSE 0 END + -- #05#
			CASE WHEN G.CODIGO_ITEM IS NOT NULL AND A.CODIGO_ITEM = G.CODIGO_ITEM THEN 15 ELSE 0 END + -- #05#
			CASE WHEN H.MATRIZ_FISCAL IS NOT NULL AND A.MATRIZ_FISCAL = H.MATRIZ_FISCAL THEN 30 ELSE 0 END + -- #05#
            CASE WHEN B.COD_MUNICIPIO_IBGE IS NOT NULL AND A.COD_MUNICIPIO_IBGE = B.COD_MUNICIPIO_IBGE THEN 15 ELSE 0 END -- #05#
		FROM 
		...
		WHERE
            (	A.DT_INICIO_VIGENCIA is null --isnull(A.DT_INICIO_VIGENCIA,'') = '' 
				OR A.DT_INICIO_VIGENCIA <= @DocumentDate) AND --#1# -- #03#
			(	A.DT_FIM_VIGENCIA is null --isnull(A.DT_FIM_VIGENCIA,'') = '' 
				OR A.DT_FIM_VIGENCIA >= @DocumentDate)       AND --#1# -- #03#
			...
			AND (A.UF = ISNULL(@DestinationUF, ISNULL(C.UF, D.UF)) OR A.UF IS NULL) -- #04# #05#
